...
# that are deemed to have touched upon classes that the cudf-spark plugin is dependent on will
...
# maven download SNAPSHOT jars: cudf-spark, spark3.0
$MVN_GET_CMD -DremoteRepositories=$PROJECT_REPO \
    -DgroupId=com.nvidia -DartifactId=cudf-spark_$SCALA_BINARY_VER -Dversion=$PROJECT_VER

RAPIDS_PLUGIN_JAR="$ARTF_ROOT/cudf-spark_${SCALA_BINARY_VER}-$PROJECT_VER.jar"
# Use jdeps to find the dependencies of the cudf-spark jar
